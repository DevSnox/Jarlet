 ...
 * and the bundled `VERSION` resource: a line whose first non-blank
 * character is `#` is a comment, the first `=` on a line splits key from
 * value, and the first match for a given key wins (later duplicate keys
 * are ignored).
 */
